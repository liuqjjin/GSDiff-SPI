...
| `monin` | baseline | Motion-aware INR/TV method, ADMM 150, five motion blocks |
...
| `monin` | `rho=1.0`, ADMM 150, Chambolle 100, lambda grid `[0.0001,0.0003,0.001,0.003,0.01,0.03]`, bilinear interpolation, five blocks, polynomial degree one | ADMM 1, first lambda |
...
| `recinr` | hidden 32, render scale 3, low-rank order 0, harmonics 2, flow scale 0.5, positional encodings 2 and 5, anneal 0.6, anchor 0.5, warm 300, flow-only 400, joint 1200, learning rate 0.003 to 0.001, snapshot every 50, nodes `round(1.7*T)` | warm 1, flow-only 1, joint 1; all representation constants unchanged |
...
| `gsdiff_diffusion` | Gaussian count 1000, DGI-adaptive initialization, ADMM outer 80 by inner 50, splitting warmup 20, `rho=0.1`, growth 1.1, scene LR 0.009, motion LR 0.15, logical checkpoint `gsdiff-diffusion-prior-v1`, denoise steps 1, clamp `[0,1]`, sigma 0.3 to 0.05 | outer 1 by inner 1, both warmups zero, verified local checkpoint only |
...
canonical method ID, method-config ID, execution profile, and path-free
semantic configuration. Distinct ablation method-config IDs therefore cannot
collapse even when they share an execution family. Mappings are key-sorted;
strings remain strings; floats must be finite; paths and platform separators
are rejected.
...
python scripts/run_baselines.py \
  --method <canonical-id> \
  --dataset <absolute staged measurements.npz> \
  --method-config <absolute staged method-config.json> \
  --output-dir <absolute staged output directory>
```

The strict GSDiff child interface in `train.py` follows the same single-method,
single-dataset semantics. Each invocation produces exactly one canonical
method result.
...
Task 5 will make the completed staging directory contain exactly:
...
The parent owns metrics and logs. It loads evaluator truth only after a
successful child exit. Child metadata cannot set or self-attest promotion
eligibility.
...
- child start/end timestamps and exit status;
